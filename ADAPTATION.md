...
上游基线是备份里的 `0.2.1`；本次适配后版本号为 **`0.2.2`**。
...
| `ctx.sessions.get(id)` / `sessions.list` 快照 store | `dsh-api-session-controller/lib/types/list.d.ts` 未变 |
...
4. **兼容性预检本身**：把 peer 范围临时改成 `0.1.4`，启动时如期出现
