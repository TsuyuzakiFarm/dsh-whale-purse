...
/** 中国法定节假日数据源（按年份取 JSON）：jsdelivr 优先，raw.githubusercontent 备用。 */
const HOLIDAY_DATA_URLS = [
...
/** 节假日数据缓存：抓取结果落盘，重启/离线时也能沿用。 */
const WHALE_PURSE_HOLIDAY_CACHE_PATH = join(process.env.DSH_HOME ?? join(homedir(), '.dsh'), 'whale-purse.holidays.json')
 ...
 */
async function fetchHolidays(years, fetchImpl = globalThis.fetch, timeoutMs = 10_000) {
  const result = {}
  for (const year of years) {
    for (const buildUrl of HOLIDAY_DATA_URLS) {
      try {
        const controller = new AbortController()
        const timer = setTimeout(() => controller.abort(), timeoutMs)
        let response
        try {
          response = await fetchImpl(buildUrl(year), { signal: controller.signal })
        } finally {
          clearTimeout(timer)
        }
...
class BalanceService {
  constructor(ctx, config = {}) {
    ...
    this.makeupWorkdaysArePeak = toBool(userSettings.makeupWorkdaysArePeak ?? config.makeupWorkdaysArePeak, false)
  ...
  loadHolidayCache() {
    try {
      if (!existsSync(WHALE_PURSE_HOLIDAY_CACHE_PATH)) return undefined
      const raw = JSON.parse(readFileSync(WHALE_PURSE_HOLIDAY_CACHE_PATH, 'utf8'))
  ...
  saveHolidayCache(cache) {
    try {
      const dir = join(WHALE_PURSE_HOLIDAY_CACHE_PATH, '..')
      if (!existsSync(dir)) mkdirSync(dir, { recursive: true })
      writeFileSync(WHALE_PURSE_HOLIDAY_CACHE_PATH, JSON.stringify(cache, null, 2), 'utf8')
    } catch (error) {
...
export function apply(ctx, config = {}) {
  ...
  const routes = [
    ...
    getRequestRoute('/api/whale-purse/balance/daily', async (req) => {
      ...
      const sessions = ctx.get('sessions')
      const list = liveSessions(sessions)
