...
/** 是否 DeepSeek 系模型：provider 或 model id 含 deepseek（官方与兼容通道都算）。 */
function isDeepSeekModel(modelId, provider) {
  if (typeof provider === 'string' && /deepseek/i.test(provider)) return true
  return typeof modelId === 'string' && /deepseek/i.test(modelId)
}

/**
 * 把 provider/model id 归一化到 flash/pro。
 * 确认非 DeepSeek（provider/model 都不含 deepseek）返回 'other' —— 调用方应跳过计价；
 * 无模型信息返回 undefined。
 */
