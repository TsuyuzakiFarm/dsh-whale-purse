 ...
 * 同时用 `inheritedEventCount` 标记 seed 的末尾 seq。seed 部分已在父会话计过费，
 * 不应再算进当前分支会话；非分支会话返回 0（从第一条事件开始计费）。
 *
 * `inheritedEventCount` 是 DSH 0.1.5 ~ 0.1.7-rc.1 都在 `Session` 上公开的字段，
 * 优先用它；`seedLength` 只是持久化 wire header 上的旧名字，留作回退。
 * @param {object} sessionLike - session、持久化 snapshot header，或 `{ events }`。
 * @returns {number} seed 边界 seq（该 seq 及之后的事件才属于本会话新产生的内容）。
 */
function seedStartSeq(sessionLike) {
  const raw = sessionLike?.inheritedEventCount
    ?? sessionLike?.seedLength
    ?? sessionLike?.header?.inheritedEventCount
...
/**
 * 读一条**非 live**（已落盘、本进程没有会话对象）会话的事件与分支 seed 边界。
 *
 * 契约（`@deepseek-ai/dsh-session-persistence`，0.1.5 ~ 0.1.7-rc.1 一致）：
 *   `list()`                     → 存储快照（`header` + `revision`）
 *   `open(id, 'read')`           → 只读句柄（不夺取写所有权）
 *   `handle.read(0)`             → `{ events }` 全量已校验事件
 *   `handle.inheritedEventCount` → 分支继承前缀长度（**header 里没有这个数**，
 *                                  它属于存储元数据，只挂在句柄上）
 *   `handle.close()`             → 释放（幂等）
 *
 * 旧实现探测的是 `persistence.readFrom(id, 0)`：这个名字在任何版本的
 * `sessionPersistence` 上都不存在，于是整段历史统计被静默跳过
 * （`coverage` 永远停在 `'live'`，「历史趋势」只剩当前进程里的会话）。
 * 这里改按真实契约读取；失败时抛出，由调用方按「跳过该会话」处理。
 *
 * @param {object} persistence - `ctx.sessionPersistence` 服务。
 * @param {string} id - 会话 id。
 * @returns {Promise<{ events: object[], boundary: number }>} 事件与 seed 边界。
 */
async function readPersistedSession(persistence, id) {
  ...
  try {
    ...
    const boundary = Number(handle.inheritedEventCount)
    return { events, boundary: Number.isFinite(boundary) && boundary > 0 ? boundary : 0 }
  } finally {
    try { await handle.close() } catch { /* 关不上不影响已读到的事件 */ }
  }
}
...
class BalanceService {
  ...
  async dailyCosts(sessionsList, days = 7) {
    ...
    // 真实契约是 `list()` + `open()`（见 readPersistedSession 的注释）；
    // 旧代码探测的 `readFrom` / `listSnapshots` 在 sessionPersistence 上都不存在。
    if (persistence !== undefined && typeof persistence.list === 'function' && typeof persistence.open === 'function') {
      try {
        ...
        for (const snapshot of snapshots) {
          ...
          // 同一 revision 的事件缓存：历史面板 10s 轮询一次，不重复解压同一份日志。
          ...
          if (cached !== undefined && snapshot.revision !== undefined && cached.revision === snapshot.revision) {
            ...
            try {
              const read = await readPersistedSession(persistence, id)
              events = read.events
              boundary = read.boundary
              this.persistedEventCache.set(key, { revision: snapshot.revision, events, boundary })
            } catch {
              // 单条会话读不出来（损坏 / 格式过新 / 权限）不该拖垮整份统计。
              continue
            }
          }
          seen.add(key)
          foldEvents(events, boundary)
        }
      } catch (error) {
        this.ctx.logger?.warn?.(error instanceof Error ? error : new Error(String(error)))
        coverage = 'live'
