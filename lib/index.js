...
function isPeakHour(now = new Date()) {
  if (!isPeakDay(now.getTime())) return false
...
/** 该北京日期是否为中国法定节假日（含调休形成的连休）。未收录年份返回 false。 */
function isChinaHoliday(epochMs) {
  return CN_HOLIDAYS.has(beijingDateKey(epochMs))
}

/**
 * 该时刻是否为“可能处于高峰”的北京工作日：周一至周五且非中国法定节假日。
 * 周末恒为 false（官方口径：周末全天空闲，调休上班的周末也不例外）。
 */
function isPeakDay(epochMs) {
  const weekday = beijingWeekday(epochMs)
  if (!['Mon', 'Tue', 'Wed', 'Thu', 'Fri'].includes(weekday)) return false
  return !isChinaHoliday(epochMs)
}
...
function nextPeakBoundary(now = new Date()) {
  ...
  for (let offset = 1; offset <= 60; offset += 1) {
    ...
    if (isPeakDay(dayNine)) return dayNine
  }
