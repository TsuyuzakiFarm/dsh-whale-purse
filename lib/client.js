...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const { useState, useEffect, useCallback, useLayoutEffect, useMemo, useRef, Fragment, useSyncExternalStore } = React
    ...
    const zh = {
      ...
      'whale-purse.proBilledAsFlash': 'Pro 按 Flash 单价计费',
      'whale-purse.proBilledAsFlashHint': 'V4.1 Flash 上线后官方把 Pro 请求路由到 Flash 并按 Flash 单价计费；关掉则按 Pro 自身价目估算',
      'whale-purse.makeupWorkdaysArePeak': '调休上班日按高峰计费',
      'whale-purse.makeupWorkdaysArePeakHint': '官方调休上班的周末（如 2026-09-20、10-10）默认仍按「周末 = 空闲时段」计；勾选后这些天按工作日 9:00-12:00 / 14:00-18:00 计高峰',
      'whale-purse.save': '保存',
    ...
    const en = {
      ...
      'whale-purse.proBilledAsFlash': 'Bill Pro at Flash rates',
      'whale-purse.proBilledAsFlashHint': 'Pro requests are served by V4.1 Flash and billed at Flash prices; turn off to price Pro with its own rates',
      'whale-purse.makeupWorkdaysArePeak': 'Bill makeup workdays at peak rates',
      'whale-purse.makeupWorkdaysArePeakHint': 'Weekend days officially designated as makeup workdays (e.g. 2026-09-20, 10-10) default to off-peak as weekends; check to treat them as weekdays (peak 9:00-12:00 / 14:00-18:00)',
    ...
    const CSS = `
      ...
      .wp-check { display: flex; align-items: flex-start; gap: 6px; font-size: 11px; opacity: .7; line-height: 1.35; }
      .wp-check input { margin: 1px 0 0; flex: none; }
      .wp-save {
    ...
    // 轮询节奏是客户端渲染参数（不是部署配置）：余额 30s、当前会话花费 3s。
    const BALANCE_POLL_MS = 30_000
    const COST_POLL_MS = 3_000
    /** 调试钩子只在带 ?whale-debug 打开页面时挂到 window 上。 */
    const DEBUG_HOOKS = typeof location !== 'undefined' && new URLSearchParams(location.search).has('whale-debug')

    /**
     * 把 `/api/...` 绝对路径解析成**文档相对** URL。
     *
     * DSH 0.1.7-rc.1 起，Web 端自己的路由（如 `/plugins/events`）一律写成
     * `new URL('...', document.baseURI)`：页面被反向代理挂在子路径下
     * （baseURI = `https://host/dsh/`）时仍然可用。插件不该假设站点根就是
     * 应用根，所以这里同样按 baseURI 解析；页面在根路径时，结果与原来的
     * 绝对路径逐字相同。
     * @param {string} path - 以 `/` 开头的应用内路径。
     * @returns {string} 可直接交给 fetch 的 URL。
     */
    ...
    /**
     * 同源 JSON GET（同源请求自动带上 /api 通道的浏览器会话 Cookie）。
     * 失败一律抛错，由调用方退成「显示上次快照」，不再猜旧路由。
     */
     ...
     * 「当前会话 id」的跨版本可观察源。
     *
     * shell.overlay 是 root scope，框架不会注入 sessionId，只能自己订阅：
     * - ≤0.1.5：`sessions.currentProvideInfo`（快照 store，字段 `sessionId`）
     * - 0.1.5 / 0.1.6：`sessions.list` 快照上的 `current`
     * - ≥0.1.7：上面两个都已移除，当前会话改由 `uiWorkspace.selection` 承载
     *   （快照 store，字段 `sessionId`，持久化名 `dsh.sessions.current`）；
     *   再退化为 `list.byId` 里被主视图保留（`retainedBy.mainView > 0`）的那条，
     *   与官方 `ui-session` 的 `publishMain()` 判定一致。
     *
     * @param {object|undefined} sessions - sessions 客户端服务。
     * @param {object|undefined} uiWorkspace - uiWorkspace 客户端服务（0.1.7+）。
     * @returns {{ subscribe: (cb: () => void) => () => void, getSnapshot: () => string|undefined }}
     */
    function currentSessionSource(sessions, uiWorkspace) {
      const stores = []
      for (const store of [sessions?.currentProvideInfo, sessions?.list, uiWorkspace?.selection]) {
        if (store !== null && store !== undefined
          && typeof store.subscribe === 'function' && typeof store.getSnapshot === 'function') stores.push(store)
      }
      /** 首个非空字符串（快照字段可能是 undefined / ''）。 */
      ...
      return {
        subscribe(callback) {
          const offs = stores.map((store) => store.subscribe(callback))
        ...
        getSnapshot() {
          ...
          if (byId !== null && typeof byId === 'object') {
            for (const summary of Object.values(byId)) {
              ...
              if ((summary.retainedBy?.mainView ?? 0) > 0 && typeof summary.id === 'string' && summary.id !== '') {
                retained = summary.id
                break
              }
            }
    ...
    /**
     * 打开（切换主视图到）某个会话：跨版本路由。
     * - ≥0.1.7：`uiWorkspace.openSession(sessionId)`（sessions 上已无 open）
     * - ≤0.1.6：`sessions.open(sessionId)`
     * @returns {string|undefined} 实际走通的通道名（测试用）。
     */
    function openSessionVia(sessions, uiWorkspace, sessionId) {
    ...
    /**
     * 「后台会话刚跑完、值得提醒一次」的判定（running 由 true 变 false 之后调用）。
     *
     * - ≤0.1.6：摘要带 `completed`（官方语义「已完成且用户尚未打开」），直接采用；
     * - ≥0.1.7：`completed` 已从摘要移除（快照字段只剩
     *   `{id,displayTitle,running,retainedBy,blank,updatedAt,…}`），用
     *   「未被主视图保留」`retainedBy.mainView === 0` 等价表达。
     *
     * @param {object|undefined} summary - `sessions.list` 快照里 byId[id] 的会话摘要。
     * @param {string} sessionId - 会话 id（回退标题用）。
     * @returns {{sessionId: string, title: string}|undefined}
     */
    function completionNoticeOf(summary, sessionId) {
      ...
      return { sessionId, title: summary.displayTitle ?? summary.title ?? sessionId }
    }

    function UsageMeter(props) {
      const t = props.t
      const sessions = props.sessions
      // 0.1.7 起「当前会话」在 uiWorkspace 上；旧版没有该服务（也不该让它变成必需依赖），
      // 因此由 apply 的 ctx.inject 按需绑定到稳定对象 runtime，这里渲染时读取现值。
      const uiWorkspace = props.runtime?.uiWorkspace ?? props.uiWorkspace
      const sessionSource = useMemo(() => currentSessionSource(sessions, uiWorkspace), [sessions, uiWorkspace])
      // shell.overlay 是 root scope，没有框架注入的 sessionId；
      // 直接订阅 sessions 服务的当前会话投影（无会话时为 undefined）。
      // 旧版 DSH：currentProvideInfo；新版 DSH（2026-08 更新起）移除该 API，
      // 当前会话 id 在 sessions.list.getSnapshot().current 上。两版都订阅，
      // 取到的 sessionId 兼容任一版本（无会话时均为 undefined）。
      const sessionId = useSyncExternalStore(sessionSource.subscribe, sessionSource.getSnapshot)
        ...
        if (prev !== null) {
          for (const [id, running] of now) {
            if (running === false && prev.get(id) === true) {
              const notice = completionNoticeOf(byId[id], id)
              if (notice !== undefined) setNotify(notice)
            }
          }
        }
      ...
      const openDoneSession = useCallback(() => {
        ...
        setNotify(null)
        try { openSessionVia(sessions, uiWorkspace, id) } catch { /* 跳转失败无妨 */ }
      }, [notify, sessions, uiWorkspace])

      // 调试/自测钩子（仅 ?whale-debug）：控制台执行
      // window.__whalePurseNotify?.('任务标题') 可直接触发提醒。
      useEffect(() => {
        if (!DEBUG_HOOKS) return undefined
        ...
        fetchJson('/api/whale-purse/settings')
          .then((snapshot) => {
            ...
            setSettingsDraft({
              ...
              proBilledAsFlash: snapshot.proBilledAsFlash ?? true,
              makeupWorkdaysArePeak: snapshot.makeupWorkdaysArePeak ?? false,
            })
      ...
      const saveSettings = useCallback(() => {
        ...
        fetch(apiUrl('/api/whale-purse/settings'), {
          method: 'POST',
          ...
          body: JSON.stringify({
            ...
            proBilledAsFlash: settingsDraft.proBilledAsFlash === true,
            makeupWorkdaysArePeak: settingsDraft.makeupWorkdaysArePeak === true,
          }),
        })
      ...
      return h(Fragment, null,
        ...
        open && panelPos !== null && h('aside', {
          ...
          settingsOpen && h('div', { className: 'wp-settings' },
            ...
            h('label', { className: 'wp-field' },
              h('span', { className: 'wp-field-label' }, t('whale-purse.proBilledAsFlash')),
              h('span', { className: 'wp-check' },
                h('input', {
                  type: 'checkbox',
                  checked: settingsDraft?.proBilledAsFlash ?? true,
                  onChange: (e) => setSettingsDraft((d) => ({ ...d, proBilledAsFlash: e.target.checked })),
                }),
                h('span', null, t('whale-purse.proBilledAsFlashHint')),
              ),
            ),
            h('label', { className: 'wp-field' },
              h('span', { className: 'wp-field-label' }, t('whale-purse.makeupWorkdaysArePeak')),
              h('span', { className: 'wp-check' },
                h('input', {
                  type: 'checkbox',
                  checked: settingsDraft?.makeupWorkdaysArePeak ?? false,
                  onChange: (e) => setSettingsDraft((d) => ({ ...d, makeupWorkdaysArePeak: e.target.checked })),
                }),
                h('span', null, t('whale-purse.makeupWorkdaysArePeakHint')),
              ),
            ),
            h('button', {
          ...
          h('footer', { className: 'wp-foot' },
            ...
            costOk && cost.pricing !== undefined && cost.pricing.peakPricingActive && cost.pricing.band !== 'standard'
              && h('div', { className: `wp-band ${String(cost.pricing.band).startsWith('off-peak') ? 'wp-band-off' : ''}` },
                String(cost.pricing.band).startsWith('peak') ? t('whale-purse.peak') : t('whale-purse.offPeak'),
                cost.pricing.nextChangeAt > 0 && h('span', null,
                  ' · ',
                  t(String(cost.pricing.band).startsWith('peak') ? 'whale-purse.nextOffPeak' : 'whale-purse.nextPeak', {
    ...
    function apply(ctx) {
      ...
      // 可选服务：0.1.7 起「当前会话」在 uiWorkspace.selection 上，旧版没有这个服务。
      // 不写进 inject（否则旧版插件直接不激活），用 ctx.inject 按需绑定到稳定对象，
      // 服务缺席时 currentSessionSource 自动退化为旧版可观察源。
      const runtime = { uiWorkspace: undefined }
      ctx.inject(['uiWorkspace'], (scope) => {
        scope.effect(() => {
          runtime.uiWorkspace = scope.uiWorkspace
          return () => { runtime.uiWorkspace = undefined }
        }, 'whale-purse: uiWorkspace seam')
      })
      ctx.inject(['slots', 'sessions'], (scope) => {
        scope.effect(() => scope.slots.register({
          ...
          order: 130,
          locale: NS,
          // runtime 身份稳定（内部字段随缝绑定变化）；uiWorkspace 再兜一层，
          // 避免依赖框架求值 inject 的时机。
          inject: () => ({ sessions: scope.get('sessions'), runtime, uiWorkspace: scope.get('uiWorkspace') }),
        }, UsageMeter), 'whale-purse: shell overlay registration')
      })
    }

    return { apply, inject, UsageMeter, currentSessionSource, openSessionVia, completionNoticeOf }
  },
})
